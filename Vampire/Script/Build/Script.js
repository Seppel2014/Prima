...
(function (Script) {
    ...
    async function start(_event) {
        ...
        light.light.color = new Script.f.Color(0.05, 0.05, 0.05);
        Script.vampire = new Script.Vampire();
        let cmpCamera = Script.viewport.getBranch().getChildrenByName("Vampire")[0].getComponent(Script.f.ComponentCamera);
    ...
    function changeLight(_value) {
        ...
        let change = color.r + 0.001 + _value;
        //console.log((1+_value-(f.Time.game.get()/1000)/maxTime))
        if (change > Script.maxDarkness) {
            let reducedColor = new Script.f.Color(change, change, change);
...
(function (Script) {
    var f = FudgeCore;
    class Vampire extends f.Node {
        vampire = Script.viewport.getBranch().getChildrenByName("Vampire")[0];
        gravity = -9.81;
        ySpeed = 0;
        isGrounded = true;
        vampireStartPoint = new f.Vector3(-2, 0.4, 0);
        xSpeed = 5;
        constructor() {
            super("Vampire");
            this.vampire.getComponent(f.ComponentRigidbody).addEventListener("ColliderEnteredCollision" /* f.EVENT_PHYSICS.COLLISION_ENTER */, this.hndCollisions);
        }
        ...
        move() {
            if (f.Keyboard.isPressedOne([f.KEYBOARD_CODE.ARROW_RIGHT, f.KEYBOARD_CODE.D])) {
                this.vampire.mtxLocal.translateX(this.xSpeed * Script.timeFrame);
            ...
            let pos = this.vampire.mtxLocal.translation;
            ...
            this.vampire.mtxLocal.translation = pos;
            this.checkCollisionWithGold(pos);
            Script.viewport.draw();
            f.AudioManager.default.update();
        }
        updateAnimation(_animation) {
            let currentAnimation = this.vampire.getChildrenByName("VampireAnimation")[0].getComponent(f.ComponentAnimator);
            let transformMtx = this.vampire.getChildrenByName("VampireAnimation")[0].getComponent(f.ComponentTransform).mtxLocal;
            let newAnimation;
            switch (_animation) {
                case "idle": {
