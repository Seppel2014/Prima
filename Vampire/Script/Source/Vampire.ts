namespace Script {
    ...
    export class Vampire extends f.Node {
    vampire: f.Node = viewport.getBranch().getChildrenByName("Vampire")[0];
    gravity: number = -9.81;
    ySpeed: number = 0;
    isGrounded: boolean = true;
    vampireStartPoint: f.Vector3 = new f.Vector3(-2,0.4,0);
    xSpeed: number = 5;
    
    constructor() {
        super("Vampire");
        this.vampire.getComponent(f.ComponentRigidbody).addEventListener(f.EVENT_PHYSICS.COLLISION_ENTER, this.hndCollisions)
    }
    ...
    public move(): void {
        if (f.Keyboard.isPressedOne([f.KEYBOARD_CODE.ARROW_RIGHT, f.KEYBOARD_CODE.D])) {
            this.vampire.mtxLocal.translateX(this.xSpeed * timeFrame);
    ...
    public updateAnimation(_animation: string): void {
        ...
        let transformMtx: f.Matrix4x4 = this.vampire.getChildrenByName("VampireAnimation")[0].getComponent(f.ComponentTransform).mtxLocal;
