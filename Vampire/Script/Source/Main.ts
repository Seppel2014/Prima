namespace Script {
  ...
  async function start(_event: CustomEvent): Promise<void> {
    ...
    light.light.color = new f.Color(0.05,0.05,0.05)

    vampire = new Vampire();
    let cmpCamera: f.ComponentCamera = viewport.getBranch().getChildrenByName("Vampire")[0].getComponent(f.ComponentCamera);
  ...
  export function changeLight(_value:number):void {
    ...
    let change:number = color.r+0.001+_value
    //console.log((1+_value-(f.Time.game.get()/1000)/maxTime))

    if(change > maxDarkness) {
      let reducedColor: f.Color = new f.Color(change,change,change);
